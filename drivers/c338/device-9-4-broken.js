...
/*    sendCommand(settings, command, callback) {
        ...
        client.write(Buffer.from(command, 'hex'));
				...
				return callback(null, devices[id].receivedData);
				}
*/
...
console.dir(this.getSettings());
    ...
    pollDevice(settings){
        ...
        nad.ampCommand(settings,command).then(response => {
        return nad.getResponse(response);
        })
               .then(data => {
